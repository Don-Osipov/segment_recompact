...
fn visible_chars_count_what_the_model_is_sent_and_nothing_else() {
    ...
    assert_eq!(visible_chars(&attachment("x1", "a2", "output_style", Some("0123456789"))), 10);
...
fn calibration_takes_the_ratio_from_the_model_and_the_live_size_from_the_last_usage() {
    ...
    assert!((1.0 / c.tokens_per_char - 2.4).abs() < 1e-9, "opus ratio from the table: {c:?}");
    assert_eq!(c.overhead, DEFAULT_OVERHEAD, "history cannot tell the resume environment's overhead");
    assert_eq!(c.live, Some(last));
    assert_eq!(c.current_tokens(&recs), last, "the compaction decision uses the live size");
    assert_eq!(c.context_tokens(&recs), DEFAULT_OVERHEAD + c.conv_tokens(&recs));

    // No usage: no live size.
    let bare: Vec<Value> = recs.iter().map(|r| {
        let mut r = r.clone();
        if let Some(m) = r.get_mut("message").and_then(|m| m.as_object_mut()) { m.remove("usage"); }
        r
    }).collect();
...
fn ceremony_session() -> Vec<Value> {
    ...
    vec![
        ...
        attachment("c1", "u1", "output_style", Some("<system-reminder>be concise</system-reminder>")),
        attachment("c2", "c1", "instructions", Some(&"CLAUDE.md contents ".repeat(200))),
        think,
        tool_use("a2", "a1", "t1", "Read", json!({"file_path": "/repo/config.toml"})),
        ...
        attachment("c3", "u2", "output_style", Some("<system-reminder>be concise</system-reminder>")),
...
fn ceremony_drops_outside_the_tail_thinking_everywhere_and_mid_turn_human_messages_never() {
    ...
    let out = load(&assemble(&dir, &src, &json!({"0": "I read /repo/config.toml."}), &[]));
    ...
    assert!(has("q1"), "a message the human typed mid-turn survives summarization verbatim");
    assert!(
        out.iter().all(|r| !serde_json::to_string(r).unwrap().contains("\"type\":\"thinking\"")),
    ...
    fs::write(&twin, out.iter().map(|r| r.to_string() + "\n").collect::<String>()).unwrap();
    assert_eq!(cmd_verify(&[twin.to_string_lossy().into_owned(), "--source".into(), src.to_string_lossy().into_owned()]), 0);
    ...
    fs::write(&bad, rechained.iter().map(|r| r.to_string() + "\n").collect::<String>()).unwrap();
    assert_eq!(cmd_verify(&[bad.to_string_lossy().into_owned(), "--source".into(), src.to_string_lossy().into_owned()]), 1);
}
...
fn own_skill_body_is_dropped_but_other_meta_records_stay() {
    ...
    let mut other = user("m2", Some("m1"), "Base directory for this skill: /x/skills/figma-to-code\n\n# figma");
    ...
    assert!(!out.iter().any(|r| r["uuid"] == "m1"), "the recompact procedure itself is dead weight");
    assert!(out.iter().any(|r| r["uuid"] == "m2"), "other skills' instructions are left alone");
}
...
fn an_oversized_final_turn_keeps_only_its_newest_parts_verbatim() {
    let mut recs = vec![user("u1", None, "first ask"), assistant("a0", "u1", "done"), user("u2", Some("a0"), "continue with the long job")];
    let mut parent = "u2".to_string();
    for i in 0..40 {
        let (tu, tr) = (format!("t{i}"), format!("r{i}"));
        recs.push(tool_use(&tu, &parent, &format!("id{i}"), "Bash", json!({"command": format!("step {i}")})));
        recs.push(tool_result(&tr, &tu, &format!("id{i}"), &format!("OUTPUT-{i} {}", "x".repeat(6000)), false));
        parent = tr;
    }
    ...
    let parts: Vec<Vec<Vec<usize>>> = segs.iter().map(|s| split_parts(&recs, s, DEFAULT_SPLIT_THRESHOLD)).collect();
    apply_tail_budget(&recs, &segs, &mut plans, &parts, 20_000);
    let tail = plans.last().unwrap();
    assert!(!tail.kept_verbatim && !tail.pinned_parts.is_empty(), "tail over budget is split");
    assert!(tail.pinned_parts.contains(&(parts.last().unwrap().len() - 1)), "the newest part is always kept");

    let out_path = dir.join("masked.jsonl");
    assert_eq!(cmd_assemble(&[src.to_string_lossy().into_owned(), "--mode".into(), "mask".into(),
        "--tail-budget".into(), "20000".into(), "--out".into(), out_path.to_string_lossy().into_owned()]), 0);
    let out = load(&out_path);
    let blob = out.iter().map(|r| r.to_string()).collect::<String>();
    assert!(blob.contains("All forty steps ran."), "newest material verbatim");
    assert!(blob.contains(&"x".repeat(6000)), "newest results verbatim");
    assert!(blob.contains("OUTPUT-0") && !blob.contains(&format!("OUTPUT-0 {}", "x".repeat(6000))),
        "the oldest part of the same turn is masked, not kept whole");
    assert!(out.iter().any(|r| text(r) == "continue with the long job"), "the human turn itself is untouched");
}
...
fn summaries_carry_files_errors_and_identifiers_the_future_uses() {
    let recs = vec![
        ...
        tool_use("a1", "u1", "e1", "Edit", json!({"file_path": "/repo/scripts/deploy.sh", "old_string": "a", "new_string": "b"})),
        tool_result("r1", "a1", "e1", "ok", false),
        tool_use("a2", "r1", "b1", "Bash", json!({"command": "psql -c 'select contract_id from t'"})),
        tool_result("r2", "a2", "b1", "ERROR: column \"contract_id\" does not exist (SQLSTATE 42703)", true),
        assistant("a3", "r2", "The flag is `CROSSPOST_MAX_GAP_DAYS` and the revision `crosspost-worker-00011-ch4`."),
        user("u2", Some("a3"), "now raise CROSSPOST_MAX_GAP_DAYS and redeploy crosspost-worker-00011-ch4"),
        assistant("a4", "u2", "Raised."),
        last_prompt("a4", "now raise it"),
    ];
    let dir = tmp_dir();
    let src = write_session(&dir, SESSION, &recs);
    let out = load(&assemble(&dir, &src, &json!({"0": "I fixed the script and found the flag."}), &[]));
    let synth = out.iter().find(|r| r["recompactSynthetic"] == true).expect("summary");
    let t = text(synth);
    assert!(t.contains("⟨carried⟩ files changed: `/repo/scripts/deploy.sh`"), "{t}");
    assert!(t.contains("SQLSTATE 42703"), "error text verbatim: {t}");
    assert!(t.contains("`CROSSPOST_MAX_GAP_DAYS`") && t.contains("`crosspost-worker-00011-ch4`"),
        "identifiers a later turn uses are carried: {t}");
    let uuid = synth["uuid"].as_str().unwrap();
    assert!(t.ends_with(&format!("[recompact summary 0 · recall {}]", &uuid[..8])), "{t}");
}

#[test]
fn inherited_footers_are_upgraded_to_the_project_wide_selector() {
    let mut old = assistant("5a5a5a5a-0000-4000-8000-000000000000", "u1", "Earlier work.\n[recompact summary 3.1 — rehydratable]");
    ...
    let recs = vec![user("u1", None, "hi"), old, user("u2", Some("5a5a5a5a-0000-4000-8000-000000000000"), "next"), assistant("a2", "u2", "ok"), last_prompt("a2", "next")];
    let dir = tmp_dir();
    let src = write_session(&dir, SESSION, &recs);
    let out = load(&assemble(&dir, &src, &json!({}), &[]));
    let t = text(out.iter().find(|r| r["recompactSynthetic"] == true).unwrap());
    assert!(t.ends_with("Earlier work.\n[recompact summary 3.1 · recall 5a5a5a5a]"), "{t}");
    assert!(!t.contains("rehydratable"), "old footer replaced, not stacked: {t}");
}
...
fn a_unit_without_agent_activity_is_summarized_mechanically() {
    ...
    let out = load(&assemble(&dir, &src, &json!({}), &[]));
    let t = text(out.iter().find(|r| r["recompactSynthetic"] == true).expect("mechanical summary"));
...
fn work_session() -> Vec<Value> {
    let mut recs = vec![
        user("u1", None, "ship the fix. never push to main directly, always open a PR"),
        tool_use("a1", "u1", "w1", "Write", json!({"file_path": "/repo/src/fix.rs", "content": "fn fix() {}"})),
        tool_result("r1", "a1", "w1", "ok", false),
        tool_use("a2", "r1", "b1", "Bash", json!({"command": "cargo test --release"})),
        tool_result("r2", "a2", "b1", "test result: ok. 12 passed; 0 failed", false),
        tool_use("a3", "r2", "b2", "Bash", json!({"command": "git checkout -b fix-branch && git commit -am 'fix it'"})),
        tool_result("r3", "a3", "b2", "[fix-branch 1a2b3c4] fix it\n 1 file changed", false),
        tool_use("a4", "r3", "b3", "Bash", json!({"command": "gh pr create --fill"})),
        tool_result("r4", "a4", "b3", "https://github.com/o/r/pull/5074", false),
        tool_use("a5", "r4", "b4", "Bash", json!({"command": "gh pr view 5074 --json state"})),
...
fn the_preamble_briefs_state_and_pins_standing_instructions_verbatim() {
    ...
    let out = load(&assemble(&dir, &src, &json!({"0": "I shipped the fix."}), &[]));
    let pre = out.iter().find(|r| r["recompactPreamble"] == true).unwrap();
    let t = text(pre);
    assert!(t.contains("State when compacted"), "{t}");
    assert!(t.contains("`/repo/src/fix.rs` (written)"), "files from code, not prose: {t}");
    ...
    assert!(t.contains("`cargo test --release` → exit 0"), "last check: {t}");
    assert!(t.contains("\"never push to main directly, always open a PR\""), "standing instruction verbatim: {t}");
    assert!(pre["recompactGeneration"] == 1 && pre["recompactAssembledAt"].is_i64());
    // The title rides along with a generation suffix.
    assert!(out.iter().any(|r| r["type"] == "custom-title" && r["customTitle"] == "Deploy fix (recompact 1)"));
    // Resume flags: the model (1M: usage ran past 200k) and the session-only effort.
    let flags = resume_flags(&work_session());
    assert_eq!(flags, vec!["--model", "claude-opus-5-5[1m]", "--effort", "max"]);
    assert_eq!(resume_command("abc", &flags), "claude --resume abc --model 'claude-opus-5-5[1m]' --effort max");
}
...
fn session_start_hook_is_silent_for_plain_sessions_and_orients_twins() {
    ...
    assert!(session_start_context(&input(&plain, "resume")).is_none(), "not a twin: say nothing");

    let twin = assemble(&dir, &plain, &json!({"0": "I shipped the fix."}), &[]);
    assert!(session_start_context(&input(&twin, "startup")).is_none(), "only resumes are oriented");
...
fn identifier_extraction_is_conservative() {
    ...
    for want in ["lib/payouts/cycle.ts", "#5074", "9f8e7d6c", "AUTHZ_PG_READ", "https://x.io/a?b=1"] {
