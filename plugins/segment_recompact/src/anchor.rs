...
use crate::{content, human_queued_text, is_genuine_user, is_human_queued, rec_type, truncate_head_tail};
...
fn looks_like_path(t: &str) -> bool {
    ...
    let ext = last
        .rsplit_once('.')
        .map(|(stem, e)| !stem.is_empty() && (1..=6).contains(&e.len()) && e.chars().all(|c| c.is_ascii_alphanumeric()))
        .unwrap_or(false);
    // A file with an extension, or a directory path of two or more segments.
    (ext && segments >= 2) || (segments >= 2 && (t.starts_with('/') || t.starts_with("~/") || t.starts_with("./")))
}
...
fn code_shaped(span: &str) -> bool {
    span.chars().any(|c| matches!(c, '_' | '.' | '/' | ':' | '-' | '#' | '(' | ' ' | '=' | '@'))
        || span.chars().any(|c| c.is_ascii_digit())
        || span.chars().skip(1).any(|c| c.is_ascii_uppercase()) && span.chars().any(|c| c.is_ascii_lowercase())
}
...
pub fn identifiers(text: &str, out: &mut Vec<String>) {
    ...
    while let Some(start) = rest.find('`') {
        ...
        match after.find('`') {
            Some(end) => {
                ...
                if (3..=80).contains(&span.len()) && !span.contains('\n') && !span.starts_with("``") && code_shaped(span) {
    ...
    for raw in text.split(|c: char| c.is_whitespace() || c == '`' || c == '"' || c == '\'' || c == ',' || c == '(' || c == ')') {
        ...
        for piece in t.split(|c: char| !is_ident_char(c) || c == '#').filter(|p| !p.is_empty()) {
            if looks_like_sha(piece) || looks_like_upper_snake(piece) {
                out.push(piece.to_string());
            }
        }
        if let Some(i) = t.find('#') {
            let tail: String = t[i..].chars().take_while(|c| *c == '#' || c.is_ascii_digit()).collect();
...
fn voiced_text(r: &Value) -> Vec<(Voice, String)> {
    ...
    match content(r) {
        Some(Value::String(s)) => v.push((if human { Voice::Human } else { Voice::Tool }, s.clone())),
...
impl Mentions {
    ...
    fn later_weight(&self, id: &str, end: usize) -> u32 {
        self.by_ident
            .get(id)
            .map_or(0, |v| v.iter().filter(|(i, _)| *i > end).map(|(_, w)| *w as u32).sum())
    }
}
...
pub fn hindsight_anchors(
    ...
    for (id, _) in ranked {
        ...
        if out.iter().any(|o: &String| o.contains(id.as_str()) || id.contains(o.as_str())) {
            continue;
        }
...
pub fn error_evidence(records: &[Value], part: &[usize], max_errors: usize, each: usize) -> Vec<(String, String)> {
    ...
    for &i in part {
        ...
        for b in blocks {
            ...
            out.push((tool, truncate_head_tail(&text, each, 0.6).replace('\n', " ⏎ ")));
...
pub fn retention(
    ...
    (kept, total, missing.into_iter().take(8).map(|(_, id)| id).collect())
}
